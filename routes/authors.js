...
router.get("/", async (req, res) => {
    try {
        ...
        res.status(400).send({ message: "Authors not found" });
    }
});
...
router.put("/:id", async (req, res) => {
    try {
        ...
        const updateAuthor = await Author.findByIdAndUpdate(its_id, req.body, {
            runValidators: true,
            new: true,
        });
...
router.delete("/:id", async (req, res) => {
    try {
        const its_id = req.params.id;
        const deleteAuthor = await Author.findByIdAndDelete(its_id);
