...
router.put("/:id", async (req, res) => {
    try {
        const its_id = req.params.id;
        const updateBook = await Book.findByIdAndUpdate(its_id, req.body, {
            runValidators: true,
            new: true,
        });
...
router.delete("/:id", async (req, res) => {
    try {
        const its_id = req.params.id;
        const deleteBook = await Book.findByIdAndDelete(its_id);
...
router.get("/:id/reviews", async (req, res) => {
    try {
        const its_id = req.params.id;
        const review = await Review.findById(its_id);
        res.status(200).send(review.book);
    } catch (error) {
